    ...
    async function loadSkills() {
      try {
        ...
        if (res.data?.success && Array.isArray(res.data.data)) {
          setDbSkills(res.data.data);
        }
      } catch (err) {
        console.warn("Could not load skills from API, falling back to static config:", err);
      }
    }
    loadSkills();
  }, []);

  const activePlanets = dbSkills.length > 0
    ? [
        techPlanets.find(p => p.orbit === 0),
        ...dbSkills.map((dbS, idx) => {
          const match = techPlanets.find(p => p.orbit !== 0 && (
            p.name.toLowerCase() === dbS.name.toLowerCase() ||
            (dbS.name === "React.js" && p.name === "React") ||
            (dbS.iconImage && p.image.endsWith(dbS.iconImage))
          ));
          if (match) return match;

          let orbitRadius = 160;
          if (dbS.category === "Frontend") orbitRadius = 80;
          else if (dbS.category === "Database") orbitRadius = 120;
          else if (dbS.category === "Backend") orbitRadius = 160;
          else orbitRadius = 200;

          return {
            id: `db-${dbS.id}`,
            name: dbS.name,
            orbit: { mobile: orbitRadius - 20, tablet: orbitRadius + 10, desktop: orbitRadius },
            size: { mobile: 20, tablet: 26, desktop: 30 },
            color: "#6366F1",
            image: dbS.iconImage ? `/skill/${dbS.iconImage}` : "/skill/developer.svg",
            speed: 10 + (idx * 2) % 15,
            offset: (idx * 45) % 360
          };
        })
